...
function renderTeams(count) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    row.innerHTML = `
      ...
      <label class="field">
        Farbe
        <input type="color" data-team-color value="${defaultColor}" />
      </label>
    `;
...
function handleStartGame() {
  ...
  const teamRows = teamListContainer?.querySelectorAll(".team-row") ?? [];
  const teams = [...teamRows].map((row, index) => {
    ...
    const colorInput = row.querySelector("[data-team-color]");
    const name = nameInput?.value.trim() || `Team ${index + 1}`;
    const icon = iconSelect?.value || TEAM_ICONS[0];
    const color = colorInput?.value || colors[index % colors.length];
    return { name, icon, color };
  });
  if (teams.length === 0) {
    for (let i = 0; i < Number.parseInt(teamCountSelect.value, 10); i += 1) {
      teams.push({
